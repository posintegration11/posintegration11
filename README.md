...
**Manual Web Service (API):** If you create the API service in the dashboard instead of the blueprint, use the same idea as `render.yaml`: install devDependencies for the build, e.g.  
`npm ci --include=dev && npm ci --prefix apps/api --include=dev && npx prisma generate && npm run build --prefix apps/api && npx prisma migrate deploy`  
Otherwise `NODE_ENV=production` causes `npm ci` to skip `@types/express` and **`tsc` fails on Render**.
